...
def run_opt(save_folder: str, grating_len: float, wg_width: float) -> None:
    ...
    trans_list = create_transformations(
        obj, monitors, 35, 65, sim_space, min_feature=80)
...
def create_sim_space(
    ...
    LAYER_SILICON_ETCHED = 103
    LAYER_SILICON_NONETCHED = 102
    ...
    waveguide_top_left = gdspy.Rectangle((-1.1 * sim_size[0] / 2, -wg_width / 2),
                                         (-grating_len / 2, wg_width / 2), LAYER_SILICON_ETCHED)
    waveguide_top_right = gdspy.Rectangle((grating_len / 2, -wg_width / 2),
                                          (1.1 * sim_size[0] / 2, wg_width / 2), LAYER_SILICON_ETCHED)
    ...
    gds_fg.add(waveguide_top_left)
    gds_fg.add(waveguide_top_right)
    gds_fg.add(waveguide_bottom)
    gds_fg.add(design_region)

    gds_bg = gdspy.Cell("BACKGROUND", exclude_from_current=True)
    gds_bg.add(waveguide_top_left)
    gds_bg.add(waveguide_top_right)
    ...
    LiNbO3_600 = optplan.Material(index=optplan.ComplexNumber(real=1.841))
    LiNbO3_1000 = optplan.Material(index=optplan.ComplexNumber(real=1.895))
    ...
    simspace = optplan.SimulationSpace(
        ...
        sim_region=optplan.Box3d(
            ...
            extents=[sim_size[0], dx, sim_z_end - sim_z_start],  # 2D sim_region
        ),
    ...
    if visualize:
        ...
        def plot(x):
            plt.imshow(np.abs(x)[:, 0, :].T.squeeze(), origin="lower")  # y-axis slice
            plt.colorbar()
...
def create_objective(
    ...
    wlens = [1535, 1540, 1545, 1546, 1548, 1550, 1552, 1554, 1555, 1560, 1565]
    wlens_obj = ["", "Min", "Max", "", "", "Max", "", "", "Max", "Min", ""]  # desire maximize/minimize output
    # wlens = [1545, 1550, 1555]
    # wlens_obj = ["Min", "Max", "Min"]  # desire maximize/minimize output
    for i, wlen in enumerate(wlens):
        ...
        wg_overlap = optplan.WaveguideModeOverlap(
            center=[grating_len / 2 + 1000, 0, wg_thickness / 2],
            extents=[0.0, 1500, 1500],
            mode_num=0,
            normal=[1.0, 0.0, 0.0],   # 和光传播的方向一致
            power=1.0,
        )
        power = optplan.abs(
        ...
        if not MINIMIZE_BACKREFLECTION:
            ...
            if wlens_obj[i] == "Max":
                obj = 1 - power
            elif wlens_obj[i] == "Min":
                obj = power
            elif wlens_obj[i] == "":
                obj = 0
            else:
                raise ValueError("illegal ‘opt_wlen’ value")
        else:
...
def create_transformations(
    ...
    cont_param = optplan.PixelParametrization(  # 似乎意思是每个pixel是个单独的变量
        simulation_space=sim_space,
    ...
    if DISCRETENESS_PENALTY:
        # Define parameters necessary to normalize discrete penalty term
